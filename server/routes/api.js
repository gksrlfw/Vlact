...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const a = await workspace.getChannels({
        include: [
          {
            model: User,
            as: "Members",
      ...
      console.log('channels', a, req.user.id);
      return res.json(
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      io.of(`/ws-${workspace.name}`)
        .to(`/ws-${workspace.name}-${channel.id}`)
...
function getKeyByValue(object, value) {
  console.log('getKey', object, value);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      console.log('onlineMap', req.params.workspace, workspace, workspace.name, onlineMap, onlineMap[`/ws-${workspace.name}`]);
      const receiverSocketId = getKeyByValue(
        onlineMap[`/ws-${workspace.name}`],
        Number(ReceiverId)
      );
      console.log('aaaaaaaa', onlineMap[`/ws-${workspace.name}`], ReceiverId);
      io.of(`/ws-${workspace.name}`)
        .to(receiverSocketId)
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const a = await channel.getMembers({
        attributes: ["id", "nickname", "email"],
      });
